...
- Amazon Bedrockを使ってLLMのAPIを呼び出す
  ...
  - しかし、今回はEC2にIAMロールを割り当てることで、DifyサーバにIAMユーザのキーを登録する必要がなくなりセキュアになります

## Difyサーバの構築

まずは、メインとなるDifyサーバを構築します。

今回は自身のAWSアカウントでDifyサーバをホストしますが、SaaSサービスとしても提供されていますので、SaaSの利用が可能なケースではよりお手軽に環境を用意できます。
セキュリティオペレーションで扱われるデータはセンシティブなものも含まれる可能性があるため、自分たちでホストする方法をご案内します。
...
先ほど有効にしたモデル `anthropic.claude-3-5-sonnet-20240620-v1:0` を入力し保存を選択します。
